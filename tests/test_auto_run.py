...
class AutomatedRunTests(unittest.TestCase):
    ...
    def test_prepare_splits_smoke_from_remaining_full_without_repeating_tasks(self) -> None:
        ...
        self.assertEqual(smoke_config["model"], "frozen-model-alias")
        self.assertEqual(smoke_config["response_model"], "frozen-model-alias")
...
def write_config(root: Path, **overrides: object) -> Path:
    config = {
        ...
        "acn_checkout": str((root / "acn").resolve()),
        ...
        "response_model": "frozen-model-alias",
